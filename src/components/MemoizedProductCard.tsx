...
export const MemoizedProductCard = memo(({ 
  ...
  }, [id, loading, toggleFavorite]);

  const handleAddToCart = useCallback(async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!stockStatus.available || isAddingToCart) return;
  ...
  }, [stockStatus.available, isAddingToCart, onAddToCart]);

  return (
    <Card 
      className="card-modern border-0 overflow-hidden group gradient-card h-full flex flex-col will-change-transform"
      role="article"
